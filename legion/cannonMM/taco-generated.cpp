...
void task_5(const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx, Runtime* runtime) {
  ...
  for (PointInDomainIterator<1> itr = PointInDomainIterator<1>(domain); itr.valid(); itr++) {
    int32_t kos = (*itr)[0];
    Point<2> bStart = Point<2>(aPartitionBounds.lo()[0], (((jn + (in + kos)) % gridX) * ((c1_dimension + (gridX - 1)) / gridX) + 0 / gridX));
    Point<2> bEnd = Point<2>(TACO_MIN(((aPartitionBounds.hi()[0] + 1) - 1), (b1_dimension - 1)), TACO_MIN((((jn + (in + kos)) % gridX) * ((c1_dimension + (gridX - 1)) / gridX) + ((c1_dimension + (gridX - 1)) / gridX - 1)), (b2_dimension - 1)));
    Rect<2> bRect = Rect<2>(bStart, bEnd);
    auto bDomain = runtime->get_index_space_domain(ctx, b_index_space);
    ...
    bColoring[(*itr)] = bRect;
    Point<2> cStart = Point<2>((((jn + (in + kos)) % gridX) * ((c1_dimension + (gridX - 1)) / gridX) + 0 / gridX), aPartitionBounds.lo()[1]);
    Point<2> cEnd = Point<2>(TACO_MIN((((jn + (in + kos)) % gridX) * ((c1_dimension + (gridX - 1)) / gridX) + ((c1_dimension + (gridX - 1)) / gridX - 1)), (c1_dimension - 1)), TACO_MIN(((aPartitionBounds.hi()[1] + 1) - 1), (c2_dimension - 1)));
    Rect<2> cRect = Rect<2>(cStart, cEnd);
    auto cDomain = runtime->get_index_space_domain(ctx, c_index_space);
...
void computeLegion(Context ctx, Runtime* runtime, LogicalRegion a, LogicalRegion b, LogicalRegion c, LogicalPartition aPartition, LogicalPartition bpart, LogicalPartition cpart, int32_t gridX) {
  ...
  for (PointInDomainIterator<2> itr = PointInDomainIterator<2>(domain); itr.valid(); itr++) {
    ...
    Point<2> bStart = Point<2>(aPartitionBounds.lo()[0], (0 / gridX));
    Point<2> bEnd = Point<2>(TACO_MIN(((aPartitionBounds.hi()[0] + 1) - 1), (b1_dimension - 1)), TACO_MIN(((gridX - 1) * ((c1_dimension + (gridX - 1)) / gridX) + ((c1_dimension + (gridX - 1)) / gridX - 1)), (b2_dimension - 1)));
    Rect<2> bRect = Rect<2>(bStart, bEnd);
    auto bDomain = runtime->get_index_space_domain(ctx, b_index_space);
    if (!bDomain.contains(bRect.lo) || !bDomain.contains(bRect.hi)) {
      bRect = bRect.make_empty();
    ...
    bColoring[(*itr)] = bRect;
    Point<2> cStart = Point<2>((0 / gridX), aPartitionBounds.lo()[1]);
    Point<2> cEnd = Point<2>(TACO_MIN(((gridX - 1) * ((c1_dimension + (gridX - 1)) / gridX) + ((c1_dimension + (gridX - 1)) / gridX - 1)), (c1_dimension - 1)), TACO_MIN(((aPartitionBounds.hi()[1] + 1) - 1), (c2_dimension - 1)));
    Rect<2> cRect = Rect<2>(cStart, cEnd);
    auto cDomain = runtime->get_index_space_domain(ctx, c_index_space);
    if (!cDomain.contains(cRect.lo) || !cDomain.contains(cRect.hi)) {
      cRect = cRect.make_empty();
