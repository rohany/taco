...
void task_4(const Task* task, const std::vector<PhysicalRegion>& regions, Context ctx, Runtime* runtime) {
  ...
  if (bDomain.get_volume() == 0 || cDomain.get_volume() == 0)
    return ;
  // std::cout << "performing blas on " << aDomain << " " << bDomain << " " << cDomain << std::endl;
...
void computeLegion(Context ctx, Runtime* runtime, LogicalRegion a, LogicalRegion b, LogicalRegion c, LogicalPartition aPartition, int32_t gridX) {
  ...
  // std::cout << "iterDomain: " << iterDomain << std::endl;
  ...
  auto bPartition = runtime->create_index_partition(ctx, b_index_space, bIKDomain, bHandColoring, LEGION_DISJOINT_COMPLETE_KIND);
  auto cPartition = runtime->create_index_partition(ctx, c_index_space, cKJDomain, cHandColoring, LEGION_DISJOINT_COMPLETE_KIND);
  ...
  public:
    ...
    LogicalRegion project(const Mappable *mappable, unsigned index,
      ...
      int gridX = ((int*)(task->args))[1];
      k = (k + point[0] + point[1]) % gridX;
      auto target = Point<2>(point[0], k);
//      std::cout << "Mapping index point: " << point << " to target point: " << target << std::endl;
  ...
  for (int32_t ko = 0; ko < gridX; ko++) {
    ...
    //   cColoring[(*itr)] = cRect;
    // }
    // auto bPartition = runtime->create_index_partition(ctx, b_index_space, domain, bColoring, LEGION_ALIASED_KIND);
    // auto cPartition = runtime->create_index_partition(ctx, c_index_space, domain, cColoring, LEGION_ALIASED_KIND);
